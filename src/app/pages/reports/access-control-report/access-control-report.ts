...
@Component({
  selector: 'app-access-control-report',
  imports: [SharedModule, CommonModule,FormsModule,],
  templateUrl: './access-control-report.html',
  styleUrl: './access-control-report.scss',
})
export class AccessControlReport {
...
if (Array.isArray(this.selectedLocationIds) && this.selectedLocationIds.length > 0 && this.selectedEmployeeId != null && this.selectedEmployeeId !== '') {
  apiUrl = 'accessControlReportEmpIdWise';
  ...
  //  date-based call
    ...
    apiUrl = 'accessControlReportDateWise';
  }

  this.dataService.addData(apiUrl, requestData).subscribe((res: any) => {

if(res.code==100){
    this.reportData = res.extend?.accessControlReport;
...
onExportPdf() {
  ...
  html2canvas(DATA, {
    ...
    pdf.save('Access Control Report.pdf');
  });
}
}

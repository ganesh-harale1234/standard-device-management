...
@Component({
  selector: 'app-person-tracking-report',
  imports: [CommonModule, FormsModule, SharedModule],
  templateUrl: './person-tracking-report.html',
  styleUrl: './person-tracking-report.scss',
})
export class PersonTrackingReport {
...
if (Array.isArray(this.selectedLocationIds) && this.selectedLocationIds.length > 0 && this.selectedEmployeeId != null && this.selectedEmployeeId !== '') {
  ...
  //  date-based call
    ...
    apiUrl = 'personTrackingReport';
  }

  this.dataService.addData(apiUrl, requestData).subscribe((res: any) => {

if(res.code==100){
    this.reportData = res.extend?.personTrackingList;
...
onExportPdf() {
  ...
  html2canvas(DATA, {
    ...
    pdf.save('Personal Traking Report.pdf');
  });
}

...
@Component({
  selector: 'app-smtp-credential',
  imports: [SharedModule, CommonModule],
  standalone: true,
  templateUrl: './smtp-credential.html',
  styleUrl: './smtp-credential.scss',
})
export class SmtpCredential implements OnInit {
...
form!: FormGroup;
 ...
 this.getallData();
  ...
  getallData(){
    this.dataService.getAllData('getAllSmtp').subscribe((res:any)=>{
      if(res.code === 100){
      this.getAllList = res.extend.AllSmtp;
      ...
      const errorMsg = err.error.msg || 'faild to SMTP Credential List !'
...
if(this.form.valid){
 ...
 this.dataService.addData('addSmtp', formData).subscribe((res:any)=>{
  if(res.code === 100){
    this.toaster.success(res.msg || 'SMTP Credential add sucessfully !')
    this.form.reset();
       this.filterallData = [...this.getAllList];
...
this.smtpId = id;
  this.showFormData = true;
    ...
    if(id){
      this.dataService.getById('getSmtpById/'+id).subscribe((res:any)=>{
        if(res.code === 100){
        const smptpData = res.extend.Smtp;
    this.form.patchValue({
    serverIp:smptpData.serverIp,
    smtpPort:smptpData.smtpPort,
    emailId:smptpData.emailId,
    password:smptpData.password,
...
onUpdate(){
  if(this.form.valid){
    ...
    this.dataService.updateData('updateSmtp',fromData).subscribe((res:any)=>{
      if(res.code === 200){
        this.toaster.success(res.msg || 'SMTP Credential Data Update Sucessfully !')
        this.getallData();
        this.form.reset();
        this.backtoList()
...
// }
  ...
  this.smtpId = id;
this.dataService.deleteData('deleteSmtpById/'+this.smtpId).subscribe((res:any)=>{
  if(res.code === 100){
 this.toaster.success(res.msg || 'Data deleted successfully !')
  ...
  applyFilter(event: any): void {
    ...
    const filtered = this.getAllList.filter((item:any) =>
      item.serverIp.toLowerCase().includes(value) ||
      item.smtpPort.toLowerCase().includes(value) 
